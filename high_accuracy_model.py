...
OPTUNA_TRIALS = 15    # Bayesian HPO trials (Fine-tuned for CPU balance)
...
def load_data() -> tuple[pd.DataFrame, np.ndarray, np.ndarray, list[str]]:
    ...
    if not csv_path.exists():
        ...
        print("  Generating synthetic dataset for immediate demonstration...")
        n_samples = 500
        feature_cols = [f"sensor_{i}" for i in range(40)]
        df = pd.DataFrame(np.random.randn(n_samples, 40), columns=feature_cols)
        df["stress_level"] = np.random.randint(1, 6, size=n_samples)
        df["participant_id"] = ["u00"] * n_samples
        df["ema_timestamp"] = pd.date_range("2013-04-01", periods=n_samples)
        df["stress_score"] = df["stress_level"].astype(float)
        df["stress_label"] = df["stress_level"].astype(str)
        # Required engineered feature columns
        df["hour_of_day"] = np.random.randint(0, 24, size=n_samples)
        df["day_of_week"] = np.random.randint(0, 7, size=n_samples)
        df["activity_active_minutes_sum"] = np.random.randint(0, 40, size=n_samples)
        df["activity_unknown_minutes_sum"] = np.random.randint(0, 20, size=n_samples)
        df["conversation_minutes_sum"] = np.random.randint(0, 50, size=n_samples)
        df["hours_of_data"] = np.ones(n_samples)
        df["phonelock_count_sum"] = np.random.randint(10, 50, size=n_samples)
        df["phonelock_minutes_sum"] = np.random.randint(10, 60, size=n_samples)
        df["audio_voice_minutes_sum"] = np.random.randint(0, 60, size=n_samples)
        df["audio_noise_minutes_sum"] = np.random.randint(0, 60, size=n_samples)
    else:
...
def build_base_models() -> dict:
    ...
    if HAS_LGB:
        models["lgb"] = LGBMClassifier(
            n_estimators=120, learning_rate=0.05, num_leaves=63,
            class_weight="balanced", n_jobs=-1,
            random_state=RANDOM_STATE, verbose=-1,
        )
    if HAS_CAT:
        models["cat"] = CatBoostClassifier(
            iterations=120, learning_rate=0.05, depth=7,
            auto_class_weights="Balanced",
...
def main() -> None:
    ...
    if HAS_CAT:
        ...
        cat_base = CatBoostClassifier(
            iterations=120, learning_rate=0.1, depth=7,
            auto_class_weights="Balanced",
            random_seed=RANDOM_STATE, verbose=0, thread_count=-1,
        )
